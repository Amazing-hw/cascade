# -*- coding: utf-8 -*-
"""
S11: Explainability reports for the cascade guard.

Outputs comparison reports, tree exports, error-path traces, and the
commercial-filter report without changing model behavior.
"""
...
PRED_COL = "cascade_pred"
BUNDLE_NAME = "corrector_bundle.pkl"
FEATURE_FILE_PREFIX = "error_features"
PALETTE = {
    "commercial": "#484878",
    "guard": "#E4CCD8",
    "delta_up": "#2E9E44",
...
def export_trees(model, out_dir):
    ...
    if model is None or not hasattr(model, "get_booster"):
        with open(os.path.join(out_dir, "all_trees.txt"), "w", encoding="utf-8") as f:
            f.write("constant_probability_guard: no tree model available\n")
        rows = [{
            "tree_id": -1,
            "json_path": "",
            "dot_path": "",
            "png_path": "",
            "note": "constant_probability_guard",
        }]
...
def export_error_paths(model, features, eval_df, feature_df, pred_col, out_dir):
    ...
    if model is None or not hasattr(model, "get_booster") or errors.empty or feature_df.empty:
...
def write_hard_negative_summary_figure(out_dir):
    fig_dir = os.path.join(out_dir, "figures")
    os.makedirs(fig_dir, exist_ok=True)
    audit_dir = os.path.join(out_dir, "hard_negative_audit")
    rows = []
    for split in ["train", "valid", "test"]:
        path = os.path.join(audit_dir, f"hard_negative_summary_{split}.csv")
    ...
    source_path = os.path.join(fig_dir, "hard_negative_summary_source.csv")
    source.to_csv(source_path, index=False)
    apply_publication_style()
    fig, ax1 = plt.subplots(figsize=(4.2, 2.45))
    if source.empty:
        ax1.text(0.5, 0.5, "No hard-negative audit data", ha="center", va="center")
        ax1.axis("off")
    else:
        x = np.arange(len(source))
        ax1.bar(x - 0.18, source["hard_negative_candidates"].astype(float), 0.36, label="Hard negatives", color=PALETTE["delta_down"], edgecolor="black", linewidth=0.4)
        ax1.bar(x + 0.18, source["worn_positive_candidates"].astype(float), 0.36, label="Worn positives", color=PALETTE["commercial"], edgecolor="black", linewidth=0.4)
        ax1.set_xticks(x)
        ax1.set_xticklabels(source["split"])
        ax1.set_ylabel("Candidate windows")
        ax1.set_title("Hard-negative audit")
        ax1.legend(loc="upper left")
        ax2 = ax1.twinx()
        ax2.plot(x, source["hard_negative_rate"].astype(float), color=PALETTE["delta_up"], marker="o", lw=1.2, label="HN rate")
        ax2.set_ylim(0, 1)
        ax2.set_ylabel("Hard-negative rate")
    fig_paths = save_publication_figure(fig, os.path.join(fig_dir, "hard_negative_summary"))
    return {"figure": fig_paths, "source_data": source_path}
...
def write_commercial_filter_report(artifact_dir, split):
    path = os.path.join(artifact_dir, f"commercial_results_{split}.csv")
    out_dir = os.path.join(artifact_dir, "commercial_filter_report")
    os.makedirs(out_dir, exist_ok=True)
    ...
    summary = {
        "rows": int(len(df)),
        "samples": int(df["sample_name"].nunique()) if "sample_name" in df else 0,
        "stage2_enabled_rows": int(df.get("stage2_enabled", pd.Series(dtype=bool)).fillna(False).sum()),
        "commercial_positive_rows": int((df.get("pred", pd.Series(dtype=int)).fillna(0).astype(int) == 1).sum()),
        "errors": int(df.get("is_error", pd.Series(dtype=int)).fillna(0).astype(int).sum()),
    }
    with open(os.path.join(out_dir, "commercial_filter_summary.json"), "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)
    df.to_csv(os.path.join(out_dir, "commercial_positive_candidates.csv"), index=False)
    with open(os.path.join(out_dir, "commercial_filter_report.md"), "w", encoding="utf-8") as f:
        f.write("# Commercial Filter Report\n\n")
...
def main():
    ...
    write_hard_negative_summary_figure(args.artifact_dir)
    if args.plot_mode == "full":
        ...
        write_commercial_filter_report(args.artifact_dir, args.split)
    else:
        print("Basic plot mode: skipped tree export, error-path tracing, and commercial filter detail report.")
