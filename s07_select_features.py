# -*- coding: utf-8 -*-
"""
S07: Stability feature selection on commercial error data.
...
def feature_selection_cache_params(
    ...
    return {
        "script": "cascade_s07_select_features",
        ...
        "commercial_score_included": True,
...
def write_feature_review(artifact_dir, train_df, valid_df, ranked, selected_features, max_features):
    ...
    for i, item in enumerate(ranked, start=1):
        ...
        auc_train = _safe_auc(train_df["should_veto"] if "should_veto" in train_df else train_df["target"], train_values)
        auc_valid = _safe_auc(valid_df["should_veto"] if "should_veto" in valid_df else valid_df["target"], valid_values)
...
def main():
    ...
    final = ensure_minimum_selected_features(
        final, stab, min_features=args.min_features, max_features=args.max_features + 1
    )
    ...
    write_feature_selection_cache(args.artifact_dir, input_paths, cache_params, final)
